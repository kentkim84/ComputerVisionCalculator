...
additons = addition_cascade.detectMultiScale(img_gray, 1.3, 5)
for (ax,ay,aw,ah) in additons:    
    cv2.rectangle(img_bgr,(ax,ay),(ax+aw,ay+ah),(255,0,0),2)

subtractions = subtraction_cascade.detectMultiScale(img_gray, 1.3, 5)
for (bx,by,bw,bh) in subtractions:
    cv2.rectangle(img_bgr,(bx,by),(bx+bw,by+bh),(0,255,0),2)

multiplications = multiplication_cascade.detectMultiScale(img_gray, 1.3, 5)
for (cx,cy,cw,ch) in multiplications:
    cv2.rectangle(img_bgr,(cx,cy),(cx+cw,cy+ch),(0,0,255),2)

divisions = division_cascade.detectMultiScale(img_gray, 1.3, 5)
